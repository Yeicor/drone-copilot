...
class FFMpegRecipe(Recipe):
    version = 'n5.1'  # ##### CHANGE: Updated to the latest release
    ...
    def build_arch(self, arch):
        with current_directory(self.get_build_dir(arch.arch)):
            ...
            # other flags:
            flags += [
                # ##### CHANGE: Enabled setpts filter
                '--enable-filter=aresample,resample,crop,adelay,volume,scale,setpts',
                '--enable-protocol=file,http,hls,tcp,udp',
                '--enable-small',
                '--enable-hwaccels',
                '--enable-pic',
                '--disable-static',
